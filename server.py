...
def status():
    ...
    return jsonify({
        ...
        "lightroom_connected": lr.is_connected(),
    })
...
@app.route("/api/lightroom/status")
def lightroom_status():
    """Get Lightroom socket connection status (for slider commands)."""
    sock = get_lightroom_socket()
    connected = sock.is_connected()
    out = {
        "connected": connected,
        "host": getattr(sock, "_active_host", sock.host),
        "port": getattr(sock, "_active_port", sock.port),
    }
    if not connected and getattr(sock, "_last_connect_error", None):
        out["last_error"] = sock._last_connect_error
    return jsonify(out)


@app.route("/api/lightroom/test", methods=["POST"])
def lightroom_test():
    """Test connection to Lightroom plugin. Helps verify plugin is running and reachable."""
    sock = get_lightroom_socket()
    # Force reconnect attempt (disconnect first so connect() tries again)
    sock.disconnect()
    ok = sock.connect()
    if ok:
        return jsonify({
            "success": True,
            "message": "Lightroom plugin is reachable. Slider commands should work.",
        })
    err = getattr(sock, "_last_connect_error", None) or "connection failed"
    return jsonify({
        "success": False,
        "error": err,
        "hint": "Is Lightroom Classic running with the Slider Control plugin enabled? See README troubleshooting.",
    }), 503
