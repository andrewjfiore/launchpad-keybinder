...
def _parse_csv_env(name: str, cast=str) -> List:
    raw = os.getenv(name, "")
...
# Multi-target (new, optional)
LIGHTROOM_SOCKET_HOSTS = _parse_csv_env("LR_SOCKET_HOSTS", str) or [
    LIGHTROOM_SOCKET_HOST,
    "127.0.0.1",
    "localhost",
]
LIGHTROOM_SOCKET_PORTS = _parse_csv_env("LR_SOCKET_PORTS", int) or [
    LIGHTROOM_SOCKET_PORT
]
...
class _LightroomSocketManager:
    def __init__(self, host: str, port: int):
        ...
        self._last_connect_error_time: float = 0.0

        # Track last good target and print throttling
        self._active_host = host
        self._active_port = int(port)
        self._last_error_print_time: float = 0.0

    def _targets(self) -> List[tuple]:
        """Return candidate (host, port) targets.

        Uses LR_SOCKET_HOSTS / LR_SOCKET_PORTS (comma separated) when present,
        otherwise falls back to the single host/port defaults.
        """
        hosts = LIGHTROOM_SOCKET_HOSTS if isinstance(LIGHTROOM_SOCKET_HOSTS, list) else [self.host, "localhost"]
        ports = LIGHTROOM_SOCKET_PORTS if isinstance(LIGHTROOM_SOCKET_PORTS, list) else [self.port]

        targets: List[tuple] = []

        # Try last known good first
        if getattr(self, "_active_host", None) and getattr(self, "_active_port", None):
            targets.append((self._active_host, int(self._active_port)))
    ...
    def _should_print_error(self, interval: float = 5.0) -> bool:
        now = time.time()
        if now - self._last_error_print_time >= interval:
    ...
    def connect(self) -> bool:
        with self._lock:
            ...
            for host, port in self._targets():
                try:
                    # create_connection handles IPv4/IPv6 resolution cleanly
                    s = socket.create_connection((host, port), timeout=1.0)

                    # Lower latency for rapid small messages
                    try:
                        s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                    except Exception:
                        pass

                    # Keep-alive helps long-running sessions on some systems
                    try:
                    ...
                    last_err = f"{host}:{port} {e}"

            self._sock = None
            self._last_connect_error = last_err or "connection failed"
            self._last_connect_error_time = time.time()
            return False
    ...
    def is_connected(self) -> bool:
        """Return True if socket is connected to Lightroom."""
        with self._lock:
    ...
    def _run(self):
        while not self._stop.is_set():
            ...
            try:
                ...
                if s is None:
                    continue
                # Plugin splits on newlines, always terminate with "\n"
...
def send_slider_to_lightroom(slider_id: str, command: str) -> bool:
    """Send Lightroom commands over the plugin TCP socket.

    Returns:
        True if the message was queued for sending, False otherwise.

    Note:
        The Lightroom plugin splits on newlines, and we keep messages newline-terminated.
    """
    ...
    ok = sock.connect()
    if not ok:
        if sock._should_print_error():
            err = getattr(sock, "_last_connect_error", None) or "connection failed"
            print(f"Lightroom socket not connected ({err}). Is the Slider Control plugin enabled in Lightroom?")
        return False
...
class PulseAnimation(LEDAnimation):
    """Pulse a pad color, then restore its baseline color."""
    ...
    def run(self):
        # Best-effort restore to the pad's baseline color at the end.
        restore_color = "off"
        try:
            restore_color = self.mapper._base_color_for_note(self.note)
        except Exception:
            restore_color = "off"

        # Pick a dim variant when available.
        if self.color in LAUNCHPAD_COLORS and self.color != "off":
            candidate = self.color + "_dim"
            dim_color = candidate if candidate in LAUNCHPAD_COLORS else self.color
        else:
            dim_color = "off"

        steps = 4
        step_duration = max(0.01, self.duration / (steps * 2))
        ...
        try:
            for _ in range(steps):
                ...
                if self.stop_event.wait(step_duration):
                    break
            ...
            # Restore baseline
            self.mapper.set_pad_color(self.note, restore_color)


class RippleAnimation(LEDAnimation):
    """A quick ripple around a pressed pad (grid only), then restore baseline LEDs."""
    def __init__(self, mapper, note: int, color: str, max_radius: int = 2, step_delay: float = 0.045):
        super().__init__(mapper, note)
        self.color = color
        self.max_radius = max(0, int(max_radius))
        self.step_delay = float(step_delay)

    def _ring_notes(self, center_note: int, radius: int) -> List[int]:
        coord = self.mapper._note_to_grid_coord(center_note)
        if coord is None:
            return []
        r0, c0 = coord
        out: List[int] = []
        for dr in range(-radius, radius + 1):
            for dc in range(-radius, radius + 1):
                if max(abs(dr), abs(dc)) != radius:
                    continue
                rr, cc = r0 + dr, c0 + dc
                n = self.mapper._grid_coord_to_note(rr, cc)
                if n is not None:
                    out.append(n)
        return out

    def run(self):
        # If not on grid, just pulse the note.
        if self.mapper._note_to_grid_coord(self.note) is None:
            self.mapper.start_animation(PulseAnimation(self.mapper, self.note, self.color, duration=0.18))
            return

        # Dim variant for rings
        ring_color = self.mapper._dim_variant(self.color)

        affected: List[int] = []
        try:
            # Center pop
            self.mapper.set_pad_color(self.note, self.color)
            if self.stop_event.wait(self.step_delay):
                return
            ...
            for radius in range(1, self.max_radius + 1):
                if self.stop_event.is_set():
                    break
                ring = self._ring_notes(self.note, radius)
                for n in ring:
                    self.mapper.set_pad_color(n, ring_color)
                affected.extend(ring)
                if self.stop_event.wait(self.step_delay):
                    break
        finally:
            # Restore baseline for affected pads
            for n in set(affected + [self.note]):
                base = self.mapper._base_color_for_note(n)
                self.mapper.set_pad_color(n, base)
...
class LaunchpadMapper:
    ...
    def __init__(self):
        ...
        self.idle_mode_active = False  # screensaver active flag
    ...
    def _idle_animation_worker(self):
        ...
        # Ensure the board starts blank so the face is the only thing lit.
        try:
            self.clear_all_pads()
        except Exception:
            pass
    ...
    def _start_idle_animation(self):
        ...
        # Screensaver behavior: blank the whole device so only the face is visible.
        try:
            self.clear_all_pads()
        except Exception:
            pass
    ...
    def send_to_lightroom(self, command: str):
        """Send a command to Lightroom using the plugin TCP socket.

        This also emits UI events so you can see what was actually sent.

        Notes:
        - Presets use lrslider:<N> which becomes a numeric command ("1".."60").
        - Repeating too fast can overwhelm Lightroom, so we rate-limit numeric commands a bit.
        """
        def _emit(evt: dict):
        ...
        # Lightweight rate-limit for incremental (numeric) commands.
        if command.isdigit():
            # Default: 80ms between identical commands (~12.5/sec). Tune via env var.
            min_interval = float(os.getenv('LR_LRSLIDER_MIN_INTERVAL', '0.08'))
            if not hasattr(self, '_lr_last_send'):
                self._lr_last_send = {}
            now = time.time()
            last = self._lr_last_send.get(command, 0.0)
            if now - last < min_interval:
                return
            self._lr_last_send[command] = now

            ok = send_slider_to_lightroom(command, command)
            if ok:
                _emit({"type": "lightroom_send", "command": command})
            else:
                sock = get_lightroom_socket()
                err = getattr(sock, "_last_connect_error", None) or "not connected"
                if sock._should_print_error():
                    _emit({"type": "lightroom_error", "error": str(err)})
            return

        # Absolute slider set commands: set_exposure:0.5, etc.
        if command.startswith('set_'):
            # Throttle per slider key (the bit after set_ and before ':')
            key = command.split(':', 1)[0]
            ok = send_slider_to_lightroom(key, command)
            if ok:
                _emit({"type": "lightroom_send", "command": command})
            else:
                sock = get_lightroom_socket()
        ...
        # Future: slider_move:Exposure:+0.1 style commands
        if command.startswith('slider_move:') or command.startswith('slider:'):
            parts = command.split(':', 2)
            slider_id = parts[1] if len(parts) >= 2 else 'slider'
            ok = send_slider_to_lightroom(slider_id, command)
            if ok:
                _emit({"type": "lightroom_send", "command": command})
            else:
                sock = get_lightroom_socket()
        ...
        # Other commands: send as-is
        ok = send_slider_to_lightroom('cmd', command)
        if ok:
            _emit({"type": "lightroom_send", "command": command})
        else:
            sock = get_lightroom_socket()
    ...
    <script>
        ...
        function startEventStream() {
            ...
            eventSource.onmessage = async (event) => {
                ...
                } else if (data.type === 'lightroom_send') {
                    log(`Lightroom ← ${data.command}`, 'success');
                } else if (data.type === 'lightroom_error') {
                    log(`Lightroom error: ${data.error}`, 'error');
...
# ============================================================================
# PERSISTENT STORAGE (profiles, presets, auto-switch settings)
# ============================================================================

APP_DIR = Path(__file__).resolve().parent
DATA_DIR = Path(os.getenv("LAUNCHPAD_MAPPER_DATA_DIR", str(Path.home() / ".launchpad_mapper"))).expanduser()
PROFILES_DIR = DATA_DIR / "profiles"
PRESETS_DIR = Path(os.getenv("LAUNCHPAD_MAPPER_PRESETS_DIR", str(APP_DIR / "presets"))).expanduser()
AUTO_SWITCH_FILE = DATA_DIR / "auto_switch.json"

for d in (DATA_DIR, PROFILES_DIR, PRESETS_DIR):
    try:
...
def _safe_stem(name: str) -> str:
    ...
    for ch in name:
        if ch.isalnum() or ch in ("_", "-", " "):
            out.append(ch)
    stem = "".join(out).strip().replace(" ", "_")
    return stem or "Default"


def _save_profile_json(profile_dict: dict) -> str:
    name = (profile_dict.get("name") or "Default").strip() or "Default"
...
def _load_profile_json(name: str) -> dict:
    stem = _safe_stem(name)
    path = PROFILES_DIR / f"{stem}.json"
    if not path.exists():
    ...
    try:
        for p in sorted(PROFILES_DIR.glob("*.json")):
            try:
                ...
                profiles.append((data.get("name") or p.stem).strip() or p.stem)
            except Exception:
                profiles.append(p.stem)
    except Exception:
        pass

    # Always include current in-memory profile
    current = (getattr(mapper.profile, "name", None) or "Default").strip() or "Default"
...
def _list_presets() -> List[dict]:
    ...
    try:
        for p in sorted(PRESETS_DIR.glob("*.json")):
            try:
                data = json.loads(p.read_text(encoding="utf-8"))
                items.append({"filename": p.name, "name": data.get("name") or p.stem})
            except Exception:
                items.append({"filename": p.name, "name": p.stem})
    except Exception:
        pass
    return items


def _load_auto_switch() -> dict:
    if AUTO_SWITCH_FILE.exists():
        try:
            return json.loads(AUTO_SWITCH_FILE.read_text(encoding="utf-8"))
        except Exception:
            return {"enabled": False, "rules": []}
    return {"enabled": False, "rules": []}


def _save_auto_switch(cfg: dict) -> None:
    payload = {
        "enabled": bool(cfg.get("enabled", False)),
        "rules": cfg.get("rules", []) if isinstance(cfg.get("rules", []), list) else [],
    }
    AUTO_SWITCH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def import_profile():
    ...
    try:
        _save_profile_json(mapper.profile.to_dict())
    except Exception:
        pass
...
def api_profiles():
    return jsonify({
        ...
        "active_profile": (mapper.profile.name or "Default").strip() or "Default",
    })
...
def api_profile_switch():
    ...
    try:
        profile_data = _load_profile_json(name)
        mapper.set_profile(Profile.from_dict(profile_data))
        return jsonify({"success": True, "active_profile": mapper.profile.name})
    except Exception as exc:
...
@app.route('/api/presets/<path:filename>')
def api_preset_get(filename):
    safe_name = Path(filename).name
    path = PRESETS_DIR / safe_name
    if not path.exists() or path.suffix.lower() != '.json':
        return jsonify({"error": "Preset not found"}), 404
    try:
        data = json.loads(path.read_text(encoding='utf-8'))
    except Exception as exc:
        return jsonify({"error": f"Invalid preset JSON: {exc}"}), 400
    return jsonify(data)


@app.route('/api/profile/auto', methods=['GET', 'POST'])
def api_profile_auto():
    # Stub: returns persisted rules, but marks feature unavailable unless you implement
    # window-title monitoring on your platform.
    available = False
    if request.method == 'POST':
        data = request.get_json(silent=True) or {}
        try:
            _save_auto_switch(data)
        except Exception as exc:
            return jsonify({"success": False, "error": str(exc), "available": available}), 400
        return jsonify({"success": True, "available": available, **_load_auto_switch()})

    cfg = _load_auto_switch()
    return jsonify({"available": available, **cfg})
...
def get_debug_midi():
    return jsonify({"success": True, "debug_midi": bool(getattr(mapper, "debug_midi", False))})
