...
def get_persistence_dir() -> Path:
    ...
    Override via LAUNCHPAD_MAPPER_DATA_DIR (e.g. for tests) to use a custom
    directory. Otherwise uses platform config: APPDATA/launchpad-mapper (Windows)
    or XDG_CONFIG_HOME/launchpad-mapper / ~/.config/launchpad-mapper (Linux/macOS).
    """
    env_dir = os.environ.get('LAUNCHPAD_MAPPER_DATA_DIR')
    if env_dir:
